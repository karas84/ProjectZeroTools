...
from typing import Generator, BinaryIO, cast
...
class CDDVD:
    ...
    def open(self, file_name: str) -> Generator[BinaryIO, None, None]:
        file_name_ = self.find_file(file_name)

        if not file_name_:
            raise FileNotFoundError(file_name_)

        with self.iso9660_facade.open_file_from_iso(f"/{file_name_}") as file_h:
    ...
    def get_file_size(self, file_name: str):
        ...
        return self.iso9660_facade.get_record(f"/{file_name_}").get_data_length()
